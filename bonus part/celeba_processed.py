...
# read datasets;
def readfile(dataset_path,thereshold=2000):
    num=0
    
    for image in sorted(os.listdir(dataset_path)):
        if int(image.replace(".jpg",'').split('_')[1])<=thereshold:
            num+=1
    ...
    for image in sorted(os.listdir(dataset_path)):
        if int(image.replace(".jpg",'').split('_')[1])<=thereshold:
            ...
            y[j] = int(image.replace(".jpg",'').split('_')[1])-1
            j+=1
    return x, y               
...
if __name__ == "__main__":
    ...
    print("Reading data")
    thereshold=100
    processed_x, processed_y = readfile(os.path.join(workspace_dir, "celeba//processed_img"),thereshold=thereshold)
    ...
    model.fc = nn.Linear(num_fc,thereshold)
    ...
    for epoch in range(num_epoch):
        ...
        if val_acc > val_acc_best:
            ...
            torch.save(model.state_dict(), 'processed_masked_resnet18_model_best.pth.tar')
            print('Save model')
